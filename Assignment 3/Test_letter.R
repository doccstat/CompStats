...
# Make sure to add column for an intercept to X and Xt
...
# Try the algorithm LRMultiClass with lambda = 1 and 50 iterations. Call the resulting object out
...
#Rprof()
#invisible(LRMultiClass(X, Y, Xt, Yt, numIter = 50, lambda = 1))
#Rprof(NULL)
#summaryRprof()

# Use microbenchmark to time your code with lambda=1 and 50 iterations. To save time, only apply microbenchmark 5 times.
library(microbenchmark)
microbenchmark(LRMultiClass(X, Y, Xt, Yt, numIter = 50, lambda = 1), times = 5)
# Report the median time of your code from bicrobenchmark above in the comments below.
# Median time:  (in sec)
# Median time is 4.168907 seconds